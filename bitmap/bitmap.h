...
protected:
    void init(int n) { M = new char[N = (n + 7) / 8]; memset(M, 0, N); } //memset 函数：将 M 后 N 个字节置为 0
public:
    ...
    void dump(char * file) { FILE * fp = fopen(file, "w"); fwrite(M, sizeof(char), N, fp); fclose(fp); }
    ...
    void expand(int k) { //若被访问的Bitmap[k]已出界，则需扩容
        ...
        init(2 * k); //与向量类似，加倍策略
        memcpy(M, oldM, oldN); //memcpy 函数：将源地址 oldM 开始拷贝 oldN 个字节到目标地址 M
        delete [] oldM; //原数据转移至新空间
    }
    
    //逐位打印以检验位图内容，非必需接口
    void print(int n) { expand(n); for (int i = 0; i < n; i++) printf(test(i) ? "1" : "0"); }
};
